...
def ev_to_img(x, y, p, e_thresh=0.15, img_size = (360, 480)):
    """
    input:
...
def create_event_imgs(events: EventBuffer, triggers=None, time_delta=5000, 
                      img_size=(360, 480), create_eimgs = True, st_t=0):
    """
    input:
    ...
    if (events is not None) and create_eimgs:
        return np.stack(eimgs), np.array(eimgs_ts, dtype=np.uint64), np.array(eimgs_ids, dtype=np.int32), np.array(trig_ids, dtype=np.int32), np.stack(eimgs_ts)
    else:
        return None, np.array(eimgs_ts, dtype=np.uint64), np.array(eimgs_ids, dtype=np.int32), np.array(trig_ids, dtype=np.int32), np.stack(eimgs_ts)
