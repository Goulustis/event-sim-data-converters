import glob
# from utils import read_intrinsics, extract_image_ids, make_camera, read_poses_bounds
from utils import read_intrinsics, make_camera, read_poses_bounds, read_intrinsics_json, extract_image_ids
from camera_spline import CameraSpline, quatList_to_poses_hom_and_tss
import numpy as np
import os
...
def main():
    ...
    # targ_dir = "ShakeCarpet1_formatted/colcam_set"
    # rgb_trigger_f = "ori_data/ShakeCarpet1/images/image_stamps_ns.txt"
    # extrinsics_f = "ori_data/ShakeCarpet1/poses_all.txt"
    # trig_ids_path = "ShakeCarpet1_formatted/ecam_set/trig_ids.npy"
    # intrinsics_f = "ori_data/ShakeCarpet1/poses_bounds.npy"

    # img_dir = "hosp_carpet/clear_coarse_frames/linear"
    # targ_dir = "hosp_carpet_formatted/colcam_set"
    # rgb_trigger_f = "hosp_carpet/clear_coarse_frames/linear/triggers.txt"
    # extrinsics_f = "ori_data/ShakeCarpet1/poses_all.txt"
    # trig_ids_path = "hosp_carpet_formatted/ecam_set/trig_ids.npy"
    # intrinsics_f = "hosp_carpet/intrinsics.json"

    src_dir = "/scratch/matthew/projects/archived/enerf/data/enerf_adapt_carpet"
    targ_dir = "./adapt_carpet_enerf/colcam_set"
    img_dir = osp.join(src_dir, "images")
    rgb_trigger_f = osp.join(src_dir, "images/image_stamps_ns.txt")
    extrinsics_f = osp.join(src_dir, "poses_all.txt")
    trig_ids_path = osp.join(osp.dirname(targ_dir), "ecam_set", "trig_ids.npy")
    intrinsics_f = osp.join(src_dir, "poses_bounds.npy")

    print("loading data")
    cam_generator = CameraSpline(extrinsics_f)
    trig_ts = (np.round(np.loadtxt(rgb_trigger_f)/1e3)).astype(int)
    # trig_ts = (np.round(np.loadtxt(rgb_trigger_f))).astype(int)
