...
parser.add_argument("--img_dir", help="path to images", default="carpet_recons/images")
parser.add_argument("--colmap_dir", help="path to colmap output [eg. colmap_dir=somepath/sparse/0 ]",default="carpet_recons/sparse/0")
parser.add_argument("--img_scale", type=int, help="the scale to rescale the scene back to", default=1)
parser.add_argument("--target_dir", help="place to save the formatted dataset", default="dev_colcam_set")
parser.add_argument("--blurry_per", help="blurry param used for filtering out blurring images", type=float, default=0)
parser.add_argument("--trigger_path", help="path to event triggers", default="image_stamps_ns.txt")
parser.add_argument("--trig_ids_path", help="path to trigger ids created from event data set", default=None)
...
triggers = (np.round(np.loadtxt(args.trigger_path)/1e3)).astype(int)

if args.trig_ids_path is None:
  trig_ids = np.array([i for i in range(len(osp.join(img_dir, "*.png")))])
else:
