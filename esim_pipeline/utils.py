...
class EventBuffer:
    def __init__(self, ev_f) -> None:
        ...
        self.t_cache = (np.array([self.t_f[0]])/1e3).astype(int)
    ...
    def update_cache(self):
        ...
        self.t_cache = np.concatenate([self.t_cache, (rt/1e3).astype(int)])
...
def make_camera(R, t, intr_mtx):
    ...
    assert np.abs(np.linalg.det(R) - 1) < 1e-3, "rotation matrix is wrong"
